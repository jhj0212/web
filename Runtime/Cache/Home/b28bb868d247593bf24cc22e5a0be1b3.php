...
<html>
	...
	<head>
		...
		<link rel="stylesheet" type="text/css" href="/RCweb/Public/Home/css/news.css" />
	...
	<body class="zhuti">
		...
		<!--neirong start-->
		<div class="neirong">
		   <?php if(is_array($newstype)): foreach($newstype as $key=>$vo): ?><div class="box1 clearfix">
					<?php if($vo["id"] == 1): ?><img src="/RCweb/Public/Home/img/5-3.png" style="width: 240px;height: 180px;"/><?php endif; ?>
					<?php if($vo["id"] == 2): ?><img src="/RCweb/Public/Home/img/5-2.png" style="width: 240px;height: 180px;"/><?php endif; ?>
					<div class="you clearfix">
						<ul>
							<li class="lei">
								<h2><?php echo ($vo["type"]); ?></h2>
								<a href="<?php echo U('more',array('pid'=>$vo['id']));?>"><p>更多></p></a>
							</li>
	                        <?php if(is_array($news)): foreach($news as $key=>$vv): if($vv['pid'] == $vo['id']): if($vv['pid'] == 1): if($i++ < 4): ?><li>
												<a href="<?php echo U('info',array('id'=>$vv['id']));?>" title="" target='_self'>
													<?php echo ($vv["title"]); ?>
												</a>
												<span><?php echo (date('Y-m-d',$vv["time"])); ?></span>
											</li><?php endif; endif; ?>
									<?php if($vv['pid'] == 2): if($j++ < 4): ?><li>
												<a href="<?php echo U('info',array('id'=>$vv['id']));?>" title="" target='_self'>
													<?php echo ($vv["title"]); ?>
												</a>
												<span><?php echo (date('Y-m-d',$vv["time"])); ?></span>
