...
{
	protected $insertFields = array('role_name','id','role_id','pri_id');
	protected $updateFields = array('id','role_name','role_id','pri_id');
...
=c.id GROUP BY a.id;*/
		$data['data'] = $this->field('a.*,GROUP_CONCAT(c.pri_name) pri_name')->alias('a')->join('LEFT JOIN db_role_privilege b ON a.id=b.role_id LEFT JOIN db_privilege c ON b.pri_id=c.id')->where($where)->group('a.id')->limit($page->firstRow.','.$page->listRows)->select();
		return $data;
	}
	// 添加前
